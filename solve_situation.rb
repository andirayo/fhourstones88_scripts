...
def analyze_brettspielnetz_game( game_number, country = :german )
=begin
=end
  printf "Analyzing game %s!\n", game_number.to_s.capitalize
...
def prepare_expect_config_and_solve( situation )
  ...
  output            = run_with_timeout( EXPECT_COMMAND, result_parser )
...
def solve_situation( situation_to_be_solved )
  best_result   = prepare_expect_config_and_solve( situation_to_be_solved )
...
unless situation_to_be_solved
  situation_to_be_solved  = analyze_brettspielnetz_game( game_number, website_type )
end #unless
...
best_result, possibilities  = solve_situation( situation_to_be_solved )
